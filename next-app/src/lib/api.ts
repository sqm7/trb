...
export async function fetchProjectNameSuggestions(countyCode: string, query: string, districts: string[] = []) {
    ...
    // Normalization: Replace Bopomofo 'ㄧ' (U+3127) with CJK '一' (U+4E00)
    // This fixes the issue where users type the phonetic symbol but data contains the numeral/kanji.
    const normalizedQuery = query.replace(/\u3127/g, '\u4E00');

    const payload = { countyCode, query: normalizedQuery, districts, detailed: true };
