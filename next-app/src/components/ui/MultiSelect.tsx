...
const MultiSelect = forwardRef<HTMLDivElement, MultiSelectProps>(({
    ...
    return (
        <div className={cn("relative w-full", className)} ref={containerRef}>
            <div
                className={cn(
                    "flex flex-wrap items-center gap-2 bg-zinc-950/50 border border-input rounded-md px-2 py-1.5 min-h-[42px] cursor-text transition-all",
            ...
            >
                {value.map((v) => {
                    ...
                    return (
                        <span key={v} className="bg-violet-500/20 text-violet-200 border border-violet-500/30 text-xs rounded-full px-2 py-0.5 flex items-center gap-1 animate-in fade-in zoom-in-95 duration-100">
                            {label}
                            {!disabled && (
                                <X
                                    size={12}
