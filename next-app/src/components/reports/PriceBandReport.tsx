...
export function PriceBandReport({ data, visibleSections = ['chart', 'table', 'location-table', 'location-chart'] }: PriceBandReportProps) {
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* 1. Filter Toggles */}
            <div id="room-type-filter-anchor" className="p-4 bg-zinc-900/30 border border-white/5 rounded-lg flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center justify-between gap-3 sm:gap-4 scroll-mt-24">
                <div className="w-full sm:w-auto flex flex-wrap items-center gap-2">
                    <span className="text-zinc-400 text-sm whitespace-nowrap shrink-0 mr-1">分析房型:</span>
                ...
                <div className="flex items-center gap-2 bg-zinc-800/50 p-1 rounded-lg border border-white/5 ml-auto sm:ml-0">
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-zinc-900/80 text-zinc-400 uppercase text-xs font-semibold">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 whitespace-nowrap min-w-[100px]">建案數</th>
                                    <th className="px-4 py-3 whitespace-nowrap">筆數</th>
                                    <th className="px-4 py-3 whitespace-nowrap">平均總價</th>
                                                ...
                                                <td className="px-4 py-3">
                                                    {item.projectNames && item.projectNames.length > 0 ? (
                                                        <button
                                                            ...
                                                            className="px-2 py-1 text-xs font-medium rounded-full bg-violet-500/20 border border-violet-500/50 text-violet-300 hover:bg-violet-500/30 transition-colors cursor-pointer whitespace-nowrap"
                                                        >
                                                            {item.projectNames.length} 建案
                                                        </button>
                                                    ) : (
