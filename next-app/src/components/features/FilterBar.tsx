...
export function FilterBar({ onAnalyze, isLoading, analysisData }: FilterBarProps) {
    // Store
    const {
        counties,
        districts,
    ...
    return (
        <div className="relative">
            {/* 1. Compact Pill Overlay (Fixed Position) */}
            <AnimatePresence>
                {isFilterBarCompact && (
                    <motion.div
                        key="compact-pill"
                        ...
                        className="fixed top-3 left-1/2 z-[100] p-1 glass-card rounded-full shadow-2xl flex items-center justify-between gap-1 transition-all mx-auto max-w-[95vw] md:max-w-7xl backdrop-blur-2xl border border-white/20 origin-top"
                    >

                        {/* Summary & Scroll Top */}
                        <div
                            className="flex items-center gap-2 pl-2 pr-2 py-1.5 hover:bg-white/5 rounded-full transition-colors cursor-pointer shrink-0"
                            onClick={() => {
                                setIsFilterBarCompact(false);
                                window.scrollTo({ top: 0, behavior: 'smooth' });
                            }}
                        >
                            <div className="h-7 w-7 rounded-full bg-violet-600/30 text-violet-300 flex items-center justify-center shrink-0 border border-violet-500/30">
                                <Filter className="h-3.5 w-3.5" />
                            </div>
                            <div className="flex flex-col leading-none overflow-hidden max-w-[100px] xs:max-w-[140px]">
                                <span className="text-[10px] text-zinc-400 font-medium truncate">{getSummaryText().split('•')[0]}...</span>
                                <span className="text-[9px] text-zinc-600 font-mono mt-0.5">點擊展開</span>
                            </div>
                        </div>

                        {/* Integrated Tabs */}
                        <div className="flex-1 min-w-0 flex items-center gap-1 px-1 border-l border-white/10 overflow-x-auto mask-fade-right pb-2 pr-8">
                            {REPORT_TABS.map((tab) => (
                                <button
                                    key={tab.value}
                                    onClick={(e) => {
                                    ...
                                    className={cn(
                                        "px-2.5 py-1 rounded-full text-[10px] font-bold transition-all whitespace-nowrap active:scale-95 touch-manipulation",
                                        activeTab === tab.value
                                            ? 'bg-violet-600 text-white shadow-lg shadow-violet-500/20'
                                            : 'text-zinc-500 hover:text-zinc-300 bg-zinc-800/20'
                                    )}
                                >
                                    {tab.label.replace('分析', '').replace('指標與', '').replace('交易', '')}
                                </button>
                            ))}
                        </div>
            ...
            {/* 2. Full Filter Bar (Always in Flow, Toggles Opacity) */}
            <div
                ref={filterRef}
                className={`p-4 md:p-6 glass-card rounded-xl shadow-lg transition-opacity duration-200 ${isFilterBarCompact ? 'opacity-0 pointer-events-none' : 'opacity-100'
                    }`}
            >
