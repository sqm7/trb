...
interface Project {
    ...
    land_plot_number: string | null;
    community_unit_count: number | null;
...
export default function AdminProjectsPage() {
    ...
    const filteredProjects = projects.filter(p =>
        ...
        p.affiliated_companies?.toLowerCase().includes(searchQuery.toLowerCase())
    );
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        ...
                                                        <div className="col-span-2">
                                                            <label className="text-[10px] text-zinc-600 font-bold block mb-1">土地使用分區 (新)</label>
                                                            <input type="text" value={p.land_use_zoning || ''} onChange={e => setProjects(prev => prev.map(x => x.id === p.id ? { ...x, land_use_zoning: e.target.value } : x))} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-300 outline-none" />
                                                        </div>
                                                        <div>
                                                            <label className="text-[10px] text-zinc-600 font-bold block mb-1">地號</label>
                                                            <input type="text" value={p.land_plot_number || ''} onChange={e => setProjects(prev => prev.map(x => x.id === p.id ? { ...x, land_plot_number: e.target.value } : x))} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-300 outline-none" />
                                                        </div>
                                                        <div>
                                                            <label className="text-[10px] text-zinc-600 font-bold block mb-1">社區戶數</label>
                                                            <input type="number" value={p.community_unit_count || ''} onChange={e => setProjects(prev => prev.map(x => x.id === p.id ? { ...x, community_unit_count: parseInt(e.target.value) || 0 } : x))} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-300 outline-none" />
                                                        </div>
                                                        <div className="col-span-2">
