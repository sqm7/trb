...
export default function LoginPage() {
  ...
  const [mode, setMode] = useState<'login' | 'register' | 'forgot_pass' | 'legacy'>('login');
  ...
  useEffect(() => {
    ...
    // Listen for auth changes to update UI without redirecting
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // User manually logged in, we can redirect or stay.
      // Current behavior: Redirect to dashboard for manual login
        ...
        <main className={`relative z-30 w-full min-h-[calc(100vh-4rem)] flex flex-col lg:flex-row items-center justify-center gap-20 p-6 lg:p-12 transition-opacity duration-1000 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="w-full max-w-md animate-in zoom-in-95 fade-in duration-700 delay-200">
            <div className="relative group">
              {/* Glow Effect */}
              <div className="absolute -inset-1 bg-gradient-to-r from-green-500 via-emerald-600 to-teal-500 rounded-2xl blur opacity-30 group-hover:opacity-60 transition duration-1000 animate-pulse"></div>

              <div className="relative bg-zinc-900/90 backdrop-blur-xl border-2 border-white/5 p-8 rounded-2xl shadow-2xl">
                ...
                {user ? (
                  ...
                  <div className="space-y-8">
                    <div className="text-center space-y-2">
                      <h2 className="text-2xl font-bold text-white">會員登入</h2>
                      <p className="text-zinc-400 text-sm">請選用 LINE 帳號快速登入</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      <button
                        ...
                        className="w-full py-4 px-4 bg-[#06C755] hover:bg-[#05b34c] text-white rounded-xl shadow-lg shadow-green-900/20 transition-all font-bold flex items-center justify-center gap-3 transform hover:-translate-y-1 relative overflow-hidden group/opt"
                      >
                        <span className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover/opt:translate-x-[100%] transition-transform duration-700 ease-in-out"></span>
                        ...
                        <span className="text-lg">LINE 一鍵登入</span>
                      </button>
                      ...
                      <div className="relative py-2">
                        ...
                        <div className="relative flex justify-center text-xs uppercase">
                          <span className="bg-zinc-900 px-2 text-zinc-600">其他選項</span>
                        </div>
                      </div>
                      ...
                      <button
                        type="button"
                        onClick={() => router.push('/dashboard')}
                        className="w-full py-3 px-4 bg-zinc-800/50 hover:bg-zinc-800 text-zinc-400 hover:text-white rounded-xl border border-zinc-700/50 hover:border-zinc-600 transition-all text-sm font-medium flex items-center justify-center gap-2 group"
                      >
                        <Zap className="w-4 h-4 text-yellow-500/80 group-hover:text-yellow-400" />
                        訪客模式 (Guest Mode)
                      </button>
                    </div>

                    {/* Hidden Legacy Login Toggle */}
                    ...
                    {/* Legacy Email Form (Hidden by default unless toggled) */}
                    {mode === 'legacy' && (
                      <form onSubmit={handleLogin} className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in slide-in-from-top-2">
                        <div className="space-y-2">
                          <input
                            ...
                            className="w-full bg-black/30 border border-zinc-800 focus:border-zinc-600 text-white rounded-lg px-4 py-2 outline-none text-sm"
                            placeholder="Email"
                          />
                          <input
                            type="password"
                        ...
                        <button type="submit" disabled={loading} className="w-full bg-zinc-800 text-white text-xs py-2 rounded hover:bg-zinc-700">
                          {loading ? 'Logging in...' : 'Sign In'}
                        </button>
                        {message && (
                          <div className={`text-xs p-2 rounded ${message.type === 'error' ? 'text-red-400 bg-red-900/20' : 'text-green-400 bg-green-900/20'}`}>
                            {message.text}
                          </div>
                        )}
                      </form>
