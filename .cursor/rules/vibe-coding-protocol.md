...
# 📋 核心文檔庫
- **SPEC.md (需求真理源)**：定義「做什麼」。包含核心邏輯、業務規則、用戶流程。
- **PLAN.md (執行路線圖)**：定義「怎麼做」。包含當前任務拆解、步驟清單、完成狀態。
- **ARCHITECTURE.md (技術架構)**：定義「在哪做」。包含目錄結構、數據庫 Schema、組件關係。
- **AGENT_SKILLS_GUIDE.md (技能說明)**：定義「AI 能力」。包含已安裝的 Skills 與 Workflows 指南。
...
在修改任何業務代碼之前，您必須：
1. 讀取 `SPEC.md` 以獲取上下文。
2. **更新 SPEC.md**：如果用戶的新需求改變了業務邏輯，先修改此文件。
3. **更新 PLAN.md**：在文件中新增一個 `### [Current Task]` 塊，將任務拆解為可量化的原子步驟（Step 1, Step 2...）。
4. **尋求確認**：向用戶展示您的計劃，詢問：「這是目前的計劃，我可以開始執行第一步嗎？」

### 第二階段：實施與記錄 (During Coding)
1. **按部就班**：嚴格按照 `PLAN.md` 的步驟執行。
...
4. **簡要總結**：告知用戶您修改了哪些文檔以及當前項目的最新狀態。
