...
export function renderPriceBandReport() {
    ...
    const tableHeaders = ['詳情', '房型', '衛浴', '筆數', '建案組成', '平均總價(萬)', '最低總價(萬)', '1/4位總價(萬)', '中位數總價(萬)', '3/4位總價(萬)', '最高總價(萬)'];
    ...
    if (filteredDataForTable.length > 0) {
        filteredDataForTable.forEach(item => { 
            const projectCompositionHtml = (item.projectNames && item.projectNames.length > 0) 
                ? item.projectNames.join('<br>') 
                : 'N/A';
            ...
            bodyHtml += `<tr class="hover:bg-dark-card transition-colors">
                ...
                <td class="scrollable-cell-y">${projectCompositionHtml}</td>
