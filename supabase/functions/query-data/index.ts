...
serve(async (req) => {
  ...
  try {
    ...
    // 【步驟 6: 擴充建案詳細資訊】
    // 查詢各縣市對應的 _projects 表格取得 土地使用分區、地號、社區戶數
    if (processedData.length > 0) {
      const uniqueProjectNames = [...new Set(processedData.map(d => d['建案名稱']).filter(Boolean))];

      if (uniqueProjectNames.length > 0) {
        const projectTableName = `${countyCode.toLowerCase()}_projects`;
        const { data: projectInfos } = await supabase
          .from(projectTableName)
          .select('project_name, land_use_zoning, land_plot_number, community_unit_count')
          .in('project_name', uniqueProjectNames);

        if (projectInfos && projectInfos.length > 0) {
          const projectMap = new Map();
          projectInfos.forEach(p => {
            projectMap.set(p.project_name, p);
          });

          processedData.forEach(row => {
            const pInfo = projectMap.get(row['建案名稱']);
            if (pInfo) {
              row['土地使用分區'] = pInfo.land_use_zoning;
              row['地號'] = pInfo.land_plot_number;
              row['社區戶數'] = pInfo.community_unit_count;
            }
          });
        }
      }
