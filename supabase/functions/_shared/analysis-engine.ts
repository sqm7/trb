...
export function calculatePriceBandAnalysis(mainData: any[]) {
    // 增強版：同時按房型+衛浴分組，並記錄各行政區的成交筆數
    const layoutGroups = new Map<string, {
        prices: number[],
        projectNames: Set<string>,
        byDistrict: Map<string, number>  // 各行政區成交筆數
    }>();

    // 用於計算整體區域統計的 Map
    const locationStats = new Map<string, Map<string, number>>(); // district -> roomType -> count
    ...
    for (const record of mainData) {
        ...
        if (!layoutGroups.has(layoutKey)) {
            layoutGroups.set(layoutKey, { prices: [], projectNames: new Set(), byDistrict: new Map() });
        }
        ...
        // 記錄行政區成交筆數
        group.byDistrict.set(district, (group.byDistrict.get(district) || 0) + 1);
        ...
        // 更新整體區域統計 (用於交叉表格)
        if (!locationStats.has(district)) {
            locationStats.set(district, new Map());
        }
        const districtMap = locationStats.get(district)!;
        districtMap.set(roomCategoryResult, (districtMap.get(roomCategoryResult) || 0) + 1);
    }

    const priceBandAnalysis = Array.from(layoutGroups.entries()).map(([layoutKey, groupData]) => {
        const { prices, projectNames, byDistrict } = groupData;
        ...
        // 將 byDistrict Map 轉換為普通物件
        const byDistrictObj: Record<string, number> = {};
        byDistrict.forEach((count, district) => {
            byDistrictObj[district] = count;
        });
        ...
        return {
            ...
            byDistrict: byDistrictObj  // 新增：各行政區成交筆數
        };
    });

    // 建立區域交叉統計表 (行=房型, 列=行政區)
    const allDistricts = Array.from(locationStats.keys()).sort();
    const allRoomTypes = Array.from(new Set(mainData.map(r => getRoomCategory(r)))).sort();

    const locationCrossTable: Record<string, Record<string, number>> = {};
    allRoomTypes.forEach(roomType => {
        locationCrossTable[roomType] = {};
        allDistricts.forEach(district => {
            const districtMap = locationStats.get(district);
            locationCrossTable[roomType][district] = districtMap?.get(roomType) || 0;
        });
    });

    return {
        details: priceBandAnalysis,
        locationCrossTable: locationCrossTable,
    ...
    for (const record of mainData) {
        ...
        if (!typeComparisonGroups.has(projectName)) {
            typeComparisonGroups.set(projectName, {
                ...
                '辦公/事務所': { sum: 0, count: 0, totalPrice: 0, totalArea: 0 },
            });
        }
        const group = typeComparisonGroups.get(projectName)!;

        // 如果之前記錄的行政區/縣市為空，但當前記錄有值，則更新
        if (!group.district && record['行政區']) group.district = record['行政區'];
        if (!group.county && record['縣市']) group.county = record['縣市'];
