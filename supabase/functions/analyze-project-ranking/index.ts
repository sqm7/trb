...
serve(async (req) => {
  ...
  try {
    ...
    for (const record of dataForRanking) {
      ...
      if (!projectGroups.has(projectName)) { projectGroups.set(projectName, { houseAreaSum: 0, transactionCount: 0, saleAmountSum: 0, housePriceSum: 0, parkingPriceSum: 0, parkingCountSum: 0, unitPrices: [], }); }
