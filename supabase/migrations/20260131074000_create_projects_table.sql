...
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            WHERE tablename = table_name AND policyname = 'Enable read access for all users'
        ) THEN
            EXECUTE format('
        ...
        -- Allow service_role full access (idempotent)
        IF NOT EXISTS (
            SELECT 1 FROM pg_policies 
            WHERE tablename = table_name AND policyname = 'Enable all access for service_role'
        ) THEN
            EXECUTE format('
        ...
        EXECUTE format('COMMENT ON TABLE public.%I IS ''建案資料表 - 縣市代碼 %s'';', table_name, city_code);
