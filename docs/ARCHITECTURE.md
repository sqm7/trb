...
### 5. 總價帶詳情表格聚合 (Detailed Table Aggregation) - Next.js (2026-01-16)

**問題**：使用者認為依「房數-衛浴數」區分的表格太過細碎，希望看到依「房型」合併的視圖，且能保持向下鑽取的能力。

**解決方案**：
- 實作前端 `Merge Bathrooms` Toggle。
- 開啟時，將數據按房型 (Room Type) 重新聚合，並計算加權平均。
- 實作「展開行 (Expandable Row)」功能，點擊合併行可查看具體的衛浴數分佈。
- 優點：同時滿足概覽與細節需求，無需後端重新查詢。
