...
## [Current Task] Fix Member Login Redirect & Deployment
- [x] Enable "Member Login" link in Mobile Header (do not hide when logged in).
- [x] Fix Page Auto-Redirect (Remove LIFF auto-push to dashboard).
- [x] Verify Landing Page "Member Login" flow.
- [x] Deploy to Test Environment (trb).
- [x] Deploy to Production Environment (kthd).

## [Current Task] Mobile Layout Optimization (RN Skills Approach)
- [ ] Step 1: Analyze & Map RN skills to Next.js (Virtualization, Animation, Touch). <!-- id: mob-1 -->
- [ ] Step 2: Audit mobile-specific CSS/Layout in `layout.tsx` and `globals.css`. <!-- id: mob-2 -->
- [ ] Step 3: Implement "Touch Target" improvements (Active states, proper sizing). <!-- id: mob-3 -->
- [ ] Step 4: Implement "List Performance" (Virtualization for DataList if needed). <!-- id: mob-4 -->
- [ ] Step 5: Implement "Animation" polish (Framer Motion / CSS Transitions). <!-- id: mob-5 -->
- [ ] Step 6: Verify mobile responsiveness. <!-- id: mob-6 -->

## [Current Task] 2026-02-01: LINE OA, Viral Snapshot & RBAC
- [x] Step 1: **Schema Design** - Created `shared_snapshots` table and added `cleanup_expired_snapshots` logic. <!-- id: viral-1 -->
- [x] Step 2: **LINE Integration** - Implemented `/auth/oa-callback` with `liff.getFriendship()` check and manual refresh. <!-- id: viral-2 -->
- [x] Step 3: **Snapshot Backend** - Created `create-snapshot` & `get-snapshot` Edge Functions with auto-expiry. <!-- id: viral-3 -->
- [x] Step 4: **Snapshot Generator UI** - Implemented `SnapshotShareModal` for Partners in Header. <!-- id: viral-4 -->
- [x] Step 5: **Snapshot Viewer UI** - Built `/share` read-only page with locked filters and conversion CTA. <!-- id: viral-5 -->
- [x] Step 6: **Auth UI Update** - Replaced standard login with LINE-first UI and hidden legacy toggle. <!-- id: viral-6 -->
- [x] Step 7: **RBAC Implementation** - Defined `profiles.role` matrix and implemented `PermissionTableModal`. <!-- id: viral-7 -->
- [x] Step 8: **Deployment Verification** - Successfully deploy to TRB (Verified at https://sqm7.github.io/trb). <!-- id: viral-8 -->

## [Current Task] FEATURE: Admin Permissions & Schema Expansion (2026-02-01)
- [x] Step 1: Implement Search Normalization (ㄧ -> 一). <!-- id: adm-1 -->
- [x] Step 2: Implement Admin Privacy (Hide SuperAdmins from Admins). <!-- id: adm-2 -->
- [x] Step 3: Create SQL Migration for multi-county project columns. <!-- id: adm-3 -->
- [x] Step 4: Update `query-data` Edge Function for dynamic project merging. <!-- id: adm-4 -->
