...
CREATE TABLE IF NOT EXISTS "public"."announcements" (
    ...
    "target_user_id" "uuid",
    CONSTRAINT "announcements_type_check" CHECK (("type" = ANY (ARRAY['info'::"text", 'warning'::"text", 'important'::"text"])))
);
...
CREATE INDEX "idx_announcements_active" ON "public"."announcements" USING "btree" ("is_active", "created_at" DESC);



CREATE INDEX "idx_announcements_pinned" ON "public"."announcements" USING "btree" ("is_pinned", "created_at" DESC);
...
ALTER TABLE ONLY "public"."announcements"
    ADD CONSTRAINT "announcements_target_user_id_fkey" FOREIGN KEY ("target_user_id") REFERENCES "auth"."users"("id");
...
CREATE POLICY "Admins can delete announcements" ON "public"."announcements" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"text", 'super_admin'::"text"]))))));



CREATE POLICY "Admins can insert announcements" ON "public"."announcements" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"text", 'super_admin'::"text"]))))));
...
CREATE POLICY "Admins can update announcements" ON "public"."announcements" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"text", 'super_admin'::"text"]))))));
...
CREATE POLICY "Admins can update profiles" ON "public"."profiles" FOR UPDATE USING (((( SELECT "profiles_1"."role"
  ...
  WHERE ("profiles_1"."id" = "auth"."uid"())) = 'super_admin'::"text") OR ((( SELECT "profiles_1"."role"
   FROM "public"."profiles" "profiles_1"
  WHERE ("profiles_1"."id" = "auth"."uid"())) = 'admin'::"text") AND ("role" <> ALL (ARRAY['admin'::"text", 'super_admin'::"text"])))));



CREATE POLICY "Admins can view all announcements" ON "public"."announcements" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"text", 'super_admin'::"text"]))))));
...
CREATE POLICY "Enable read access for all users" ON "public"."announcements" FOR SELECT USING ((("is_active" = true) AND (("target_user_id" IS NULL) OR ("target_user_id" = "auth"."uid"()))));
